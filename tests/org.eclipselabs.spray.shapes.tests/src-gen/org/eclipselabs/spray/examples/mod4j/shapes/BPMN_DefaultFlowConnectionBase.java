...
public class BPMN_DefaultFlowConnectionBase extends DefaultSprayConnection {
	...
	public static enum TextIds {
	   ...
	   public BPMN_DefaultFlowConnectionBase(final IFeatureProvider fp) {
	super(fp);
	}
	...
	protected ConnectionDecorator createDecorator_2(Diagram diagram, Connection connection, ISprayStyle sprayStyle) {
		ConnectionDecorator decorator_2 = peCreateService.createConnectionDecorator(connection, false, 0.05, true);
		List<Point> pointList = new ArrayList<Point>();
		pointList.add(gaService.createPoint(0, -10, 0, 0));
		pointList.add(gaService.createPoint(5, 10, 0, 0));
		Polyline element = gaService.createPolyline(decorator_2, pointList);
		ISprayStyle style = sprayStyle;
		element.setStyle(style.getStyle(diagram));
		return decorator_2;
	}
